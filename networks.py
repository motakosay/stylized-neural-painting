...
class PixelShuffleNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_show = x.reshape((int(np.sqrt(len(x))), int(np.sqrt(len(x)))))


        x_show = x_show.detach().numpy()
...
class ZouFCNFusion(nn.Module):
    ...
    def forward(self, x):
        ...
        x_alpha_show = x_alpha.detach().numpy()
        x_alpha_show = x_alpha_show * mask_show

        color_show = color.squeeze().permute(1,2,0)

        color_show = color_show.detach().numpy()

        result = color_show * mask_show

        plt.imshow(x_alpha_show), plt.title('x_alpha * mask')
        plt.show()

        plt.imshow(color_show), plt.title('color')
        plt.show()
